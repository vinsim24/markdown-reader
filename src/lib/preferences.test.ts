...
import { contrastRatio, defaultPreferences, loadPreferences, validatePreferences } from './preferences';
...
describe('preferences', () => {
  ...
  it('applies theme fonts unless the user explicitly chose a font', () => {
    expect(validatePreferences({ ...defaultPreferences, theme: 'mono', font: 'inter' }).font).toBe('jetbrains');
    expect(validatePreferences({ ...defaultPreferences, theme: 'mono', font: 'literata', fontExplicit: true }).font).toBe('literata');
  });
